...
RTC_ISABDEV_PLATFORM_DATA_BEGIN(rtc_data)
...
void board_init()
{
	board_power_init(); //电源初始化
	adc_vbg_init();     //只返回return，没有作用
	adc_init();         //ADC初始化
	cfg_file_parse(0);  //配置初始化
	devices_init();     //UPDATE初始化


	board_devices_init();   //PWMLED,ADKEY,充电，RTC初始化

	if(get_charge_online_flag()) {
		power_set_mode(PWR_LDO15);
	} else {
		power_set_mode(TCFG_LOWPOWER_POWER_SEL);//这里TCFG_LOWPOWER_POWER_SEL = PWR_LDO15，库函数，设置电源模式为PWR_LDO15
	}

	/*close FAST CHARGE */
#if TCFG_UART0_ENABLE   //串口打印
	if (uart0_data.rx_pin < IO_MAX_NUM) {
		gpio_set_die(uart0_data.rx_pin, 1);
	}
#endif

#if USER_UART_UPDATE_ENABLE     //0
        {
...
static void port_wakeup_callback(u8 index, u8 gpio)
{
	...
	switch (index) {
#if TCFG_TEST_BOX_ENABLE    //充电仓配置(不支持) 0
		case 2:
...
void board_power_init(void)
{
    ...
    power_init(&power_param);   //低功耗配置，库函数
    ...
    power_set_callback(TCFG_LOWPOWER_LOWPOWER_SEL, sleep_enter_callback, sleep_exit_callback, board_set_soft_poweroff);//库函数

//    wl_audio_clk_on();

    power_keep_dacvdd_en(0);//库函数

	power_wakeup_init(&wk_param);   //唤醒初始化，库函数

    aport_edge_wkup_set_callback(aport_wakeup_callback);    //库函数，与充电有关
    port_edge_wkup_set_callback(port_wakeup_callback);  //库函数，与充电有关

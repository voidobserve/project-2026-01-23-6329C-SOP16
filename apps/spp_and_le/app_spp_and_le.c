...
extern u8 Ir_Key_Val;
extern OS_SEM phaseSequenceKeySem;
//---------------------------------------------------------------------
#if TRANS_CLIENT_EN
//指定搜索uuid
//指定搜索uuid
...
{
//    printf("------------%s---------------\n", __FUNCTION__);       //打印功能函数名
    __set_user_ctrl_conn_num(TCFG_BD_NUM);  //连接的设备数，1个设备
    __set_support_msbc_flag(1);
#if BT_SUPPORT_DISPLAY_BAT  //是否使能电量检测，0
...
#if (TCFG_BD_NUM == 2)  //连接设备个数配置，只连接1个设备
    __set_auto_conn_device_num(2);
#endif
#if BT_SUPPORT_MUSIC_VOL_SYNC   //是否使能音量同步，0
    vol_sys_tab_init();
#endif
    ...
    __set_simple_pair_param(3, 0, 2);   //库函数

#if TCFG_USER_BLE_ENABLE    //BLE功能使能，1
    {
        u8 tmp_ble_addr[6];
        /* le_controller_set_mac((void*)"012345"); */
#if DOUBLE_BT_SAME_MAC  //同地址，0
        //the same
        memcpy(tmp_ble_addr, bt_get_mac_addr(), 6);
#else
        lib_make_ble_address(tmp_ble_addr, (void *)bt_get_mac_addr());//将MAC地址转换成BLE地址
#endif
...
#if TRANS_CLIENT_EN     //蓝牙(ble主机)透传，0
...
#if(TCFG_USER_EDR_ENABLE && TRANS_DATA_EN)  //EDR功能使能，蓝牙双模透传，0
...
#if (USER_SUPPORT_PROFILE_SPP==1)   //不支持SPP0
...
#if (TCFG_USER_EDR_ENABLE || TCFG_USER_BLE_ENABLE)  //ble:1
    u32 sys_clk =  clk_get("sys");
    bt_pll_para(TCFG_CLOCK_OSC_HZ, sys_clk, 0, 0);  //设置蓝牙的时钟

    bt_function_select_init();  //蓝牙功能初始化，这里只用到BLE
    bredr_handle_register();    //SPP,电量检测，音量同步寄存器设置，这里都没用上
...
#if TCFG_USER_EDR_ENABLE    //EDR功能使能，0
...
#endif
    /* 按键消息使能 */
    sys_key_event_enable(); //库函数
    /* sys_auto_shut_down_enable(); */
    /* sys_auto_sniff_controle(1, NULL); */
    /* app_timer_handle  = sys_timer_add(NULL, app_timer_handler, 500); */

//    task_create(led_refresh_task_handle, NULL, "led_refresh");  //创建灯条刷新任务
    led_main(); //应用主程序入口
}
...
#if TCFG_TEST_BOX_ENABLE    //充电仓配置(不支持)
...
//    printf("------------%s---------------\n", __FUNCTION__);       //打印功能函数名
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        if(event->u.key.type == KEY_DRIVER_TYPE_IR)     //单击
        {
            extern LED_STATE led_state;
            if(led_state.running_task == PHASE_SEQUENCE)        //相序调整单独按键处理
            {
                Ir_Key_Val = key_value;
                os_sem_post(&phaseSequenceKeySem);
            }
            else                                                //其他模式通用按键处理
                os_taskq_post("ir_key", 1,key_value);              //通过消息队列把键值传回按键处理函数
        }
...
#if TCFG_CHARGE_ENABLE  //是否支持芯片内置充电,0

...
void mcpwm_set_frequency(pwm_ch_num_type ch, pwm_aligned_mode_type align, u32 frequency)
{
    ...
    reg->tmr_con |= (i << 3); //div 2^i分频，2分频后是12M
    mcpwm_div_clk = clk / (1 << i); //2分频后是12M
    if (frequency == 0) {
        mcpwm_tmr_pr = 0;
    } else {
        if (align == pwm_center_aligned) { //中心对齐
            ...
            mcpwm_tmr_pr = mcpwm_div_clk / frequency - 1;   //tmr0 pr = 12000000/1000-1=11999
        }
    }
    reg->tmr_pr = mcpwm_tmr_pr; //把这个值写到寄存器
    //timer mode
    if (align == pwm_center_aligned) { //中心对齐
        reg->tmr_con |= 0b10;   //递增递减循环
    } else {
        reg->tmr_con |= 0b01;   //递增模式
    }
}
...
{
    PWM_TIMER_REG *timer_reg = get_pwm_timer_reg(pwm_ch);   //读取定时器控制寄存器地址
    PWM_CH_REG *pwm_reg = get_pwm_ch_reg(pwm_ch);   //读取PWM控制寄存器地址

    if (pwm_reg && timer_reg) {
        pwm_reg->ch_cmpl = timer_reg->tmr_pr * duty / 10000;
        pwm_reg->ch_cmph = pwm_reg->ch_cmpl;
        timer_reg->tmr_cnt = 0;     //重新开始计数
        timer_reg->tmr_con |= 0b01;
        if (duty == 100) {    //如果占空比=10000，不需要计数，直接一直保持
            timer_reg->tmr_cnt = 0;
...
//    pwm_p_data.h_pin = IO_PORTB_09;                         //PB9
    pwm_p_data.h_pin = IO_PORTA_00;                         //PA0
    ...
    pwm_p_data.frequency = 1000;                            //1KHz
    pwm_p_data.duty = 0;                                 //占空比50%
//    pwm_p_data.h_pin = IO_PORTA_01;                         //H引脚PA1
    pwm_p_data.h_pin = IO_PORTA_01;                         //H引脚PA1
    pwm_p_data.l_pin = -1;                                  //不需要
    ...
    pwm_p_data.duty = 0;                                 //占空比50%
//    pwm_p_data.h_pin = IO_PORTA_02;                         //H引脚PA2
    pwm_p_data.h_pin = IO_PORTB_07;                         //H引脚Pb7
    pwm_p_data.l_pin = -1;                                  //任意引脚,不需要就填-1
    mcpwm_init(&pwm_p_data);
#endif
#if PWM_CH3_ENABLE
    ...
    pwm_p_data.frequency = 10000;                           //10KHz
    pwm_p_data.duty = 0;                                 //占空比75%
    pwm_p_data.h_pin = IO_PORTA_04;                         //H引脚PA4
    pwm_p_data.l_pin = IO_PORTA_05;                         //L引脚PA5
    pwm_p_data.complementary_en = 0;                        //两个引脚的波形, 0: 同步,  1: 互补，互补波形的占空比体现在H引脚上
    mcpwm_init(&pwm_p_data);
#endif
...
//    mcpwm_close(redLED_CH);   //灭灯
//    mcpwm_close(blueLED_CH);
 //   mcpwm_close(greenLED_CH);
}
...
void set_duty(JL_TIMER_TypeDef *JL_TIMERx, u32 duty)
{
    u32 u_clk = 24000000;
    //初始化timer
    JL_TIMERx->CON = 0;
    ...
    JL_TIMERx->CON |= (0b0001 << 4);					//时钟源再4分频
    JL_TIMERx->CNT = 0;									//清计数值
    JL_TIMERx->PRD = u_clk / (4 * 1000);				    //设置周期
    JL_TIMERx->PWM = (JL_TIMERx->PRD * duty) / 10000;	//设置初始占空比，0~10000对应0~100%
    JL_TIMERx->CON |= (0b01 << 0);						//计数模式
    JL_TIMERx->CON |= BIT(8);							//PWM使能
}
